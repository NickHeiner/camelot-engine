...
function isValidMove(gameState, moveParts) {
    ...
    function isValidMoveRec(gameState, moveParts, jumpedPlayer, nonJumpHasOccurred) {
        ...
        if (srcBoardSpace.player === constants.PLAYER_A && destBoardSpace.row === constants.PLAYER_A_GOAL_ROW) {
            return false;
        }
        if (srcBoardSpace.player === constants.PLAYER_B && destBoardSpace.row === constants.PLAYER_B_GOAL_ROW) {
            return false;
        }
        ...
        if (spaceBetween !== null) {
            ...
            if (jumpedPlayer !== null &&
                boardSpaceBetween.player !== jumpedPlayer &&
                srcBoardSpace.piece !== constants.KNIGHT) {
                return false;
            }
            jumpedPlayer = boardSpaceBetween.player;
        } else {
            if (jumpedPlayer !== null) {
                return false;
            }
