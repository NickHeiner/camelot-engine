...
function isValidMove(gameState, moveParts) {
    ...
    function isValidMoveRec(gameState, moveParts, moveContext) {
        ...
        if (spaceBetween !== null && !getBoardSpace(gameState, spaceBetween).piece) {
            return false;
        }

        gameAfterFirstMove = applyMove(gameState, moveParts[0], moveParts[1]);
        return isValidMoveRec(gameAfterFirstMove, _.rest(moveParts));

    }

    return isValidMoveRec(gameState, moveParts, {});
