...
function makePieceRow(gameState, countPawns, row, colStart, player) {
  ...
  const pieces = [constants.KNIGHT].concat(repeat(constants.PAWN, countPawns)).concat([constants.KNIGHT]),
    colOffsets = _.range(countPawns + 2);

  return _.reduce(colOffsets, function(gameStateAcc, colOffset) {
